...
import { RiLoopLeftLine, RiSparklingFill, RiUpload2Line } from "@remixicon/react"

export default function Form({ onSubmit }: { onSubmit: (data: FormData) => void }) {
    ...
    return (
        ...
        >
            ...
            >
                ...
                >
                    ...
                    <span className="font-bold text-xs uppercase text-neutral-600">
                        {file ? (
                            <>
                                {file.name} <RiLoopLeftLine className="inline-block h-4 align-text-bottom ml-3" /> Заменить
                            </>
                        ) : (
                            <>
                                <RiUpload2Line className="inline-block h-4 align-text-bottom" /> Загрузить видео
                            </>
                        )}
                    </span>
                </motion.label>
            ...
            <motion.label
                ...
                animate={{ opacity: 1, scale: 1, y: 0, transition: { delay: 0.5 } }}
            >
                <span className="font-bold text-xs uppercase text-neutral-600">Заголовок видео</span>
                <motion.input
                    name="title"
                    ...
                    whileFocus={{ y: -2 }}
                    placeholder="Соник в кино 3 смотреть полный фильм рутуб"
                />
            </motion.label>
            <motion.label
                className="flex flex-col gap-1 w-full"
                ...
                animate={{ opacity: 1, scale: 1, y: 0, transition: { delay: 0.5 } }}
            >
                <span className="font-bold text-xs uppercase text-neutral-600">Описание видео</span>
                <motion.textarea
                    name="description"
                    ...
                    whileFocus={{ y: -2 }}
                    placeholder={"Приятного просмотра!\nПереходи по ссылке и получай 100 бонусов в 1XBET:"}
                />
            </motion.label>
            <motion.div
                className="w-full"
